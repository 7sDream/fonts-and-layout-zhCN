...
自从20世纪60年代以来，计算机显示领域发生的另一个巨大变化是支持了多色显示。emoji 的大量出现也让我们希望字体能够显示出多种颜色的#tr[glyph]，这就意味着现在字体需要决定#tr[glyph]用什么颜色来绘制。在此之前这都是由应用程序来控制的。
...
OpenType 为在字体中嵌入彩色/#tr[bitmap]图片提供了多种选择：灰度#tr[bitmap]图、彩色#tr[bitmap]图、彩色字形、“Apple格式彩色图片”、SVG。你可以在字体里混合使用上述的多种技术，来最大化应用程序兼容性。比如在本书写作时，如果字体同时含有彩色#tr[outline]（`COLR`）和 SVG，则Firefox可以显示其中的 SVG，在还不支持 SVG 字体的 IE 和 Chrome 中则可以显示 `COLR` 彩色字形。彩色#tr[bitmap]图（`CBDT`）格式则只用在Google公司Android系统中的emoji上。
