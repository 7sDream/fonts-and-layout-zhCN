...
#show: web-page-template.with(title: "参考资料")
...
#let bibentry(key, body, verb: [编写的]) = (
  cite(key, form: "author") + [ ] + verb + body + cite(key, form: "normal")
)

- 拉丁文#tr[diacritic]设计：
  // - [The Insects Project](http://theinsectsproject.eu) - a downloadable book on issues of Central European diacritic design
  - #bibentry(
      <Balik.InsectsProject.2016>,
    )[《The Insects Project》，一本关于中欧#tr[diacritic]设计的可下载电子书]
  ...
  - #bibentry(
      <Gaultney.ProblemsDiacritic.2008>,
  ...
  - #bibentry(
      <Twardoch.PolishDiacritics.1999>,
    )[《Polish diacritics how-to》文档]
  // - Filip Blažek's [Diacritics project](http://diacritics.typo.cz)
  - #bibentry(
      <Blazek.DiacriticsProject.2006>,
      verb: [制作的],
    )[Diacritics project 网站]
...
- #bibentry(
    <Truong.VietnameseTypography>,
  )[关于越南文#tr[typography]的在线电子书《Vietnamese Typography》]
...
- 关于特定#tr[character]的设计指导：
  ...
  - #bibentry(
      <PabloImpallari.TcommaTcedilla.2013>,
      verb: [],
    )[在TypeDrawers论坛上关于 Tcomma 和 Tcedilla 的提问帖]
...
- 阿拉伯文资源：
  ...
  - #bibentry(
      <Kew.NotesUnicode.2005>,
      verb: [],
    )[向Unicode提交的提案《Notes on some Unicode Arabic characters: recommendations for usage》]
...
- 印度系#tr[scripts]资源：
  ...
  - #bibentry(
      <IndianTypeFoundry.AnnotatedFeature.2015>,
    )[具有完善注释的天城文字体特性文件]
// * Script databases:
- #tr[scripts]数据库：
