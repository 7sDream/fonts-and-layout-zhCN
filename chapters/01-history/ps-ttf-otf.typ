...
#show: web-page-template.with(title: "从 PostScript 到 TrueType")

// ## PostScript Fonts, TrueType and OpenType
// 这里和原文意思不一样是有意的
// 主要是因为本段其实没有 OpenType 的相关内容
// 而且作为标题过长，酌情修改
== 从PostScript到TrueType
...
提前看到在字体中运用PostScript程序的潜力的字体设计师并不多，其中一个著名的例子是Erik van Blokland和Just van Rossum设计的*FF Beowolf*（@figure:beowolf）。他们没有直接使用PostScript的`lineto`和`curveto`指令来绘制曲线和直线，而是自己实现了一个`freakto`指令，它使用随机数生成器来扭曲点的位置。每当需要绘制一个#tr[character]时，字体就会调用随机数生成器，并生成一个新的设计。这其实解构了字体的概念，毕竟在我们的印象中，字体里的每个#tr[character]总是以相同的样子出现。
...
那时，苹果和微软尝试与Adobe建立各种合作，但都被拒之门外——Adobe把PostScript Type 1当作掌上明珠。于是在1987年他们决定反击，开始共同开发一种可缩放的字体格式。这一格式为#tr[rasterization]和屏幕显示而设计，而且在操作系统中内置了相应的#tr[rasterization]程序。1989年，Apple出售了他们手中Adobe的全部股份，并在旧金山的Seybold桌面出版大会上公开发布了TrueType格式。字体大战开始了。#footnote[#cite(form: "prose", <Shimada.FontWars.2006>)是对那个时代的极好的历史回顾。]
